...
async def make_safe_query(client: AsyncClient, url: str) -> dict:
    ...
    while True:
        if tries >= MAX_TRIES:
            raise HTTPException(
                500, "API server may not be available. Max retries reached..."
            )
        try:
...
def parse_result(
    result: dict, type: Query_Type
) -> Character_Response | Location_Response:
    if type == "character":
...
async def get_paginated_results(
    client: AsyncClient, url: str, type: Query_Type
) -> list[Character_Data | Location_Data]:
    ...
    return first_result.results + [
        r for sublist in parsed_results for r in sublist.results
    ]
...
async def get_data() -> list[Character_Data]:
    ...
    try:
        ...
        print(
            "Something went wrong with reading redis data. Querying again agains API..."
        )
        print(e)

    async with AsyncClient(timeout=10) as client:
        # First list all possible locations that are "Earth"
        earth_locations = await get_paginated_results(
            client, BASE_API + "/location?name=Earth", "location"
        )
        earth_fixed_names = [x.name for x in earth_locations]

        characters = await get_paginated_results(
            client, BASE_API + "/character?species=human&status=alive", "character"
        )
