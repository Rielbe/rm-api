...
@character_router.get(
    "/earth_characters",
    responses={400: {"model": ErrorResponse, "description": "Invalid user input"}},
)
async def get_earth_characters(
    request: Request,
    sort_by: list[str] | None = Query(
        None,
        description=f"Allowed values: {', '.join(list(Character_Data.model_fields.keys()))}",
    ),
) -> list[Character_Data]:
    if sort_by:
    ...
    sorted_characters = sorted(earth_characters, key=attrgetter(*sort_by))
...
async def get_status():
    return {
        "Server cache available": cache_available(),
        "Cache is ready": await cache_is_ready(),
